...
// import
import { sendTodo } from "../../utilites/redux/Action/todolist";
...
const FormAddNew = (props) => {
  ...
  const handlerSubmit = (e) => {
   ...
   //  validation the input
    if((inputElement.current.value.trim() ==null && inputDesc.current.value.trim() ==null)) return toast.error("You must fill all data ", { position: toast.POSITION.TOP_CENTER });
    ...
    // making request and added loading
    setIsLoading(true)
    dispatch(sendTodo(dataTodo,setIsLoading));
    //   close modal
      ...
      <div className="mt-4">
        <form onSubmit={handlerSubmit}>
          ...
          <div className="mt-4">
            <button
              type="submit"
              className="inline-flex font-mainFont tracking-wider justify-center rounded-md border border-transparent bg-emerald-100 px-4 py-2 text-sm font-medium text-emerald-900 hover:bg-emerald-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2"
            >
              Save
            </button>
          </div>
